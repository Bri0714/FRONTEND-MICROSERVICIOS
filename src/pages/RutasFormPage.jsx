// src/pages/RutasFormPage.jsx

import { useState, useEffect } from "react";
import { useForm } from "react-hook-form";
import { useNavigate } from "react-router-dom";
import { createRuta } from "../api/rutas.api";
import { createVehiculo } from "../api/vehiculos";
import { createConductor } from "../api/conductores";
import { getAllInstituciones } from "../api/instituciones.api";
import { toast } from "react-hot-toast";
import { Footer } from "../components/footer";

export function RutasFormPage() {
        ...
        if (currentStep === 1) {
            ...
            try {
                ...
                if (error.response && error.response.data) {
                    ...
                    toast.error("Error al crear la ruta", { position: "bottom-right" });
                }
            } finally {
                setIsLoading(false);
            }
            ...
            try {
                ...
                setVehiculoPlaca(response.data.vehiculo_placa);
                toast.success("Vehículo y Monitora creados exitosamente", {
                    position: "bottom-right",
                });
                setCurrentStep(4);
            } catch (error) {
                if (error.response && error.response.data) {
                    ...
                    toast.error("Error al crear el vehículo y la monitora", {
                        position: "bottom-right",
                    });
                }
            } finally {
                    ...
                    {currentStep === 2 && (
                        <>
                            ...
                            <button
                                    type="submit"
                                    disabled={isLoading}
                                    className={`bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg w-full transition-all duration-300 ease-in-out transform hover:-translate-y-1 hover:scale-105 focus:outline-none focus:ring-4 focus:ring-blue-300`}
                                >
                                    Siguiente
                                </button>
                            </div>
                            ...
                            <div className="mt-6 flex justify-between">
                                ...
                                <button
                                    ...
                                    className={`${
                                        ...
                                        aceptaTerminosVehiculo ? "hover:-translate-y-1 hover:scale-105" : ""
                                    } focus:outline-none focus:ring-4 focus:ring-blue-300`}
                                >
                                    {isLoading
                                        ? "Guardando..."
                                        : "Guardar Vehículo y Monitora"}
                                </button>
                            </div>
                        </>
                    )}
                            ...
                            <button
                                    ...
                                    className={`${
                                        aceptaTerminosConductor
                                            ? "bg-blue-500 hover:bg-blue-700"
                                            : "bg-gray-500 cursor-not-allowed"
                                    } text-white font-bold py-2 px-4 rounded-lg w-full transition-all duration-300 ease-in-out transform ${
                                        aceptaTerminosConductor ? "hover:-translate-y-1 hover:scale-105" : ""
                                    } focus:outline-none focus:ring-4 focus:ring-blue-300`}
                                >
                                    {isLoading ? "Guardando..." : "Guardar Conductor"}
                                </button>
                            </div>
                        </>
                    )}
